# Congo DR (COD) — チーム分析レポート

> **グループ K** ｜ 主戦フォーメーション: `4-2-3-1` ｜ 平均OVR: **74.1** ｜ 分析試合数: 7試合 (StatsBomb)

---
## 1. 選手層

### ゴールキーパー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Lionel Mpasi | Le Havre | **67** | None | None | None | None | None | None |
| Timothy Fayulu | Noah | **—** | — | — | — | — | — | — |
| Matthieu Epolo | Standard Liège | **—** | — | — | — | — | — | — |

### ディフェンダー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Aaron Wan-Bissaka | West Ham United | **80** | 80 | 51 | 69 | 77 | 78 | 73 |
| Chancel Mbemba 🅲 | Lille | **75** | 67 | 56 | 65 | 67 | 75 | 73 |
| Arthur Masuaku | Lens | **75** | 79 | 62 | 74 | 78 | 68 | 79 |
| Joris Kayembe | Genk | **74** | 79 | 60 | 71 | 74 | 66 | 74 |
| Axel Tuanzebe | Burnley | **73** | 76 | 36 | 64 | 68 | 73 | 75 |
| Gédéon Kalulu | Aris Limassol | **—** | — | — | — | — | — | — |

### ミッドフィールダー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Noah Sadiki | Sunderland | **74** | 77 | 65 | 68 | 76 | 68 | 76 |
| Ngal'ayel Mukau | Lille | **74** | 69 | 57 | 65 | 71 | 70 | 75 |
| Edo Kayembe | Watford | **71** | 72 | 66 | 69 | 70 | 67 | 78 |
| Charles Pickel | Espanyol | **71** | 63 | 63 | 68 | 69 | 67 | 80 |
| Samuel Moutoussamy | Atromitos | **—** | — | — | — | — | — | — |
| Gaël Kakuta | AEL | **—** | — | — | — | — | — | — |

### フォワード

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Yoane Wissa | Newcastle United | **82** | 85 | 82 | 70 | 80 | 31 | 71 |
| Cédric Bakambu | Real Betis | **77** | 78 | 78 | 68 | 72 | 40 | 68 |
| Meschak Elia | Alanyaspor | **70** | 82 | 68 | 67 | 75 | 26 | 70 |
| Théo Bongonda | Spartak Moscow | **—** | — | — | — | — | — | — |
| Fiston Mayele | Pyramids | **—** | — | — | — | — | — | — |

---
## 2. キープレーヤー

### Yoane Wissa (OVR 82)
- **ポジション**: FW  |  **クラブ**: Newcastle United
- **年齢**: 29  |  **代表キャップ**: 36  |  **代表得点**: 9
- **能力値**: PAC 85 / SHO 82 / PAS 70 / DRI 80 / DEF 31 / PHY 71

### Aaron Wan-Bissaka (OVR 80)
- **ポジション**: DF  |  **クラブ**: West Ham United
- **年齢**: 28  |  **代表キャップ**: 10  |  **代表得点**: 0
- **能力値**: PAC 80 / SHO 51 / PAS 69 / DRI 77 / DEF 78 / PHY 73

### Cédric Bakambu (OVR 77)
- **ポジション**: FW  |  **クラブ**: Real Betis
- **年齢**: 35  |  **代表キャップ**: 68  |  **代表得点**: 21
- **能力値**: PAC 78 / SHO 78 / PAS 68 / DRI 72 / DEF 40 / PHY 68

### Chancel Mbemba (OVR 75)
- **ポジション**: DF  |  **クラブ**: Lille
- **年齢**: 31  |  **代表キャップ**: 107  |  **代表得点**: 7
- **能力値**: PAC 67 / SHO 56 / PAS 65 / DRI 67 / DEF 75 / PHY 73

### Arthur Masuaku (OVR 75)
- **ポジション**: DF  |  **クラブ**: Lens
- **年齢**: 32  |  **代表キャップ**: 44  |  **代表得点**: 4
- **能力値**: PAC 79 / SHO 62 / PAS 74 / DRI 78 / DEF 68 / PHY 79

---
## 3. 戦術的特徴

### 主フォーメーション: 4-2-3-1
> 守備的MF2枚を軸に、3列目に広さを持つバランス型。サイドアタックと中盤のプレス強度が高い。

**使用フォーメーション分布:**
- `4-2-3-1` : 17回
- `4-4-1-1` : 2回
- `4-4-2` : 1回

### 主な先発メンバー（StatsBomb実績）

- Chancel Mbemba Mangulu
- Samuel Moutoussamy
- Lionel Mpasi-Nzau
- Gedeon Kalulu Kyatengwa
- Arthur Masuaku
- Charles Pickel
- Yoane Wissa
- Théo Bongonda Mbul'Ofeko Batombo
- Cédric Bakambu
- Henoc Inonga Baka
- Gaël Kakuta

### よく途中出場する選手

- Aaron Tshibola
- Silas Katompa Mvumpa
- Simon Bokote Banza
- Meschak Elia
- Fiston Kalala Mayele
- Omenuke Mfulu

### 近年の主な試合結果

| 大会 | 対戦相手 | スコア | フォーメーション |
|------|----------|:------:|:---------------:|
| AFCON 2023 | Egypt | 1-1 | 4-2-3-1 |
| AFCON 2023 | Zambia | 1-1 | 4-2-3-1 |
| AFCON 2023 | Morocco | 1-1 | 4-2-3-1 |
| AFCON 2023 | South Africa | 0-0 | 4-2-3-1 |
| AFCON 2023 | Côte d'Ivoire | 0-1 | 4-2-3-1 |
| AFCON 2023 | Guinea | 3-1 | 4-4-1-1 |
| AFCON 2023 | Tanzania | 0-0 | 4-2-3-1 |

---
## 4. 試合展開の有利・不利

### ✅ 有利になりやすい状況

- 2ボランチによる守備的な安定感があり、ボール奪取後の素早い縦パスでカウンターを発動しやすい
- 3列目の横幅を使い、サイドチェンジで相手ブロックをこじ開けることができる

### ⚠️ 不利になりやすい状況

- 相手が5バックを敷くと中盤が窮屈になり、ビルドアップが詰まりやすい
- 1トップの孤立が起きると前線の起点が失われ、攻撃が停滞する
- GKのクオリティに不安があり、ミドルシュートやセットプレーで失点しやすい

---
## 5. グループステージ展望

グループ **K** の対戦相手: Portugal, Uzbekistan, Colombia

_（予測スコアは schedule.json 入力後に更新予定）_
