# Colombia (COL) — チーム分析レポート

> **グループ K** ｜ 主戦フォーメーション: `4-1-2-1-2` ｜ 平均OVR: **75.9** ｜ 分析試合数: 6試合 (StatsBomb)

---
## 1. 選手層

### ゴールキーパー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| David Ospina | Atlético Nacional | **76** | None | None | None | None | None | None |
| Álvaro Montero | Vélez Sarsfield | **75** | None | None | None | None | None | None |
| Camilo Vargas | Atlas | **—** | — | — | — | — | — | — |

### ディフェンダー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Davinson Sánchez | Galatasaray | **82** | 82 | 58 | 63 | 73 | 82 | 88 |
| Daniel Muñoz | Crystal Palace | **81** | 75 | 69 | 72 | 76 | 79 | 81 |
| Johan Mojica | Mallorca | **78** | 88 | 65 | 73 | 73 | 74 | 70 |
| Yerry Mina | Cagliari | **77** | 34 | 50 | 52 | 55 | 77 | 84 |
| Jhon Lucumí | Bologna | **76** | 65 | 38 | 62 | 61 | 77 | 78 |
| Santiago Arias | Independiente | **—** | — | — | — | — | — | — |

### ミッドフィールダー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Richard Ríos | Benfica | **79** | 81 | 70 | 77 | 82 | 72 | 82 |
| Jhon Arias | Palmeiras | **78** | 85 | 75 | 74 | 79 | 42 | 68 |
| Jefferson Lerma | Crystal Palace | **77** | 68 | 71 | 70 | 74 | 76 | 79 |
| Juan Fernando Quintero | River Plate | **77** | 62 | 76 | 79 | 81 | 41 | 54 |
| Kevin Castaño | River Plate | **77** | 76 | 68 | 76 | 76 | 72 | 75 |
| Jorge Carrascal | Flamengo | **70** | 66 | 63 | 70 | 65 | 69 | 71 |
| James Rodríguez 🅲 | Minnesota United FC | **56** | 67 | 56 | 53 | 57 | 30 | 46 |

### フォワード

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Luis Díaz | Bayern Munich | **85** | 88 | 81 | 76 | 87 | 45 | 75 |
| Luis Suárez | Sporting CP | **77** | 83 | 77 | 66 | 75 | 38 | 78 |
| Jhon Córdoba | Krasnodar | **69** | 80 | 59 | 62 | 70 | 27 | 55 |
| Cucho Hernández | Real Betis | **—** | — | — | — | — | — | — |

---
## 2. キープレーヤー

### Luis Díaz (OVR 85)
- **ポジション**: FW  |  **クラブ**: Bayern Munich
- **年齢**: 29  |  **代表キャップ**: 72  |  **代表得点**: 21
- **能力値**: PAC 88 / SHO 81 / PAS 76 / DRI 87 / DEF 45 / PHY 75

### Davinson Sánchez (OVR 82)
- **ポジション**: DF  |  **クラブ**: Galatasaray
- **年齢**: 29  |  **代表キャップ**: 77  |  **代表得点**: 3
- **能力値**: PAC 82 / SHO 58 / PAS 63 / DRI 73 / DEF 82 / PHY 88

### Daniel Muñoz (OVR 81)
- **ポジション**: DF  |  **クラブ**: Crystal Palace
- **年齢**: 30  |  **代表キャップ**: 44  |  **代表得点**: 3
- **能力値**: PAC 75 / SHO 69 / PAS 72 / DRI 76 / DEF 79 / PHY 81

### Richard Ríos (OVR 79)
- **ポジション**: MF  |  **クラブ**: Benfica
- **年齢**: 25  |  **代表キャップ**: 30  |  **代表得点**: 2
- **能力値**: PAC 81 / SHO 70 / PAS 77 / DRI 82 / DEF 72 / PHY 82

### Johan Mojica (OVR 78)
- **ポジション**: DF  |  **クラブ**: Mallorca
- **年齢**: 33  |  **代表キャップ**: 43  |  **代表得点**: 1
- **能力値**: PAC 88 / SHO 65 / PAS 73 / DRI 73 / DEF 74 / PHY 70

---
## 3. 戦術的特徴

### 主フォーメーション: 4-1-2-1-2
> ダブルFWとトップ下を縦に並べた縦志向。ワンツーパスと積極的なプレスで速攻を狙う。

**使用フォーメーション分布:**
- `4-1-2-1-2` : 8回
- `4-3-3` : 2回
- `4-2-3-1` : 1回
- `4-4-1-1` : 1回
- `3-5-1-1` : 1回

### 主な先発メンバー（StatsBomb実績）

- Camilo Andrés Vargas Gil
- Davinson Sánchez Mina
- Richard Rios Montoya
- Jhon Adolfo Arias Andrade
- James David Rodríguez Rubio
- Luis Fernando Díaz Marulanda
- Daniel Muñoz Mejía
- Carlos Eccehomo Cuesta Figueroa
- Jefferson Andrés Lerma Solís
- Jhon Andrés Córdoba Copete
- Johan Andrés Mojica Palacio

### よく途中出場する選手

- Andrés Mateus Uribe Villa
- Kevin Duvan Castaño Gil
- Rafael Santos Borré Amaury
- Jorge Andrés Carrascal Guardo
- Luis Fernando Sinisterra Lucumí
- Juan Fernando Quintero Paniagua

### 近年の主な試合結果

| 大会 | 対戦相手 | スコア | フォーメーション |
|------|----------|:------:|:---------------:|
| Copa America 2024 | Brazil | 1-1 | 4-1-2-1-2 |
| Copa America 2024 | Paraguay | 2-1 | 4-1-2-1-2 |
| Copa America 2024 | Panama | 5-0 | 4-1-2-1-2 |
| Copa America 2024 | Argentina | 0-1 | 4-1-2-1-2 |
| Copa America 2024 | Uruguay | 1-0 | 4-2-3-1 |
| Copa America 2024 | Costa Rica | 3-0 | 4-1-2-1-2 |

---
## 4. 試合展開の有利・不利

### ✅ 有利になりやすい状況

- 1ボランチを軸に前後左右へのサポートが速く、素早いパス交換でプレスを回避できる
- 2トップのコンビネーションがはまると、ペナルティエリア内での崩しが鋭い

### ⚠️ 不利になりやすい状況

- アンカー脇のスペースを突かれると中盤がこじ開けられ、DF裏への一発が通りやすい
- 両SBが攻撃参加した際のサイドカバーが薄くなり、失点リスクが高まる

---
## 5. グループステージ展望

グループ **K** の対戦相手: Portugal, Congo DR, Uzbekistan

_（予測スコアは schedule.json 入力後に更新予定）_
