# Egypt (EGY) — チーム分析レポート

> **グループ G** ｜ 主戦フォーメーション: `4-3-3` ｜ 平均OVR: **79.0** ｜ 分析試合数: 4試合 (StatsBomb)

---
## 1. 選手層

### ゴールキーパー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Mohamed El Shenawy | Al Ahly | **—** | — | — | — | — | — | — |
| Mostafa Shobeir | Al Ahly | **—** | — | — | — | — | — | — |
| Mohamed Alaa | El Gouna | **—** | — | — | — | — | — | — |

### ディフェンダー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Mohamed Abdelmonem | Nice | **74** | 62 | 40 | 63 | 61 | 74 | 74 |
| Ramy Rabia | Al Ain | **67** | 60 | 49 | 55 | 55 | 67 | 72 |
| Hamdy Fathy | Al-Wakrah | **—** | — | — | — | — | — | — |
| Mohamed Hany | Al Ahly | **—** | — | — | — | — | — | — |
| Ahmed Fatouh | Zamalek | **—** | — | — | — | — | — | — |
| Yasser Ibrahim | Al Ahly | **—** | — | — | — | — | — | — |
| Hossam Abdelmaguid | Zamalek | **—** | — | — | — | — | — | — |

### ミッドフィールダー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Marwan Attia | Al Ahly | **—** | — | — | — | — | — | — |
| Emam Ashour | Al Ahly | **—** | — | — | — | — | — | — |
| Mohanad Lasheen | Pyramids | **—** | — | — | — | — | — | — |
| Mahmoud Saber | ZED | **—** | — | — | — | — | — | — |

### フォワード

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Mohamed Salah 🅲 | Liverpool | **91** | 89 | 88 | 86 | 90 | 45 | 76 |
| Omar Marmoush | Manchester City | **84** | 89 | 85 | 76 | 86 | 34 | 71 |
| Trézéguet | Al Ahly | **—** | — | — | — | — | — | — |
| Zizo | Al Ahly | **—** | — | — | — | — | — | — |
| Ibrahim Adel | Nordsjælland | **—** | — | — | — | — | — | — |
| Haissem Hassan | Oviedo | **—** | — | — | — | — | — | — |

---
## 2. キープレーヤー

### Mohamed Salah (OVR 91)
- **ポジション**: FW  |  **クラブ**: Liverpool
- **年齢**: 33  |  **代表キャップ**: 116  |  **代表得点**: 67
- **能力値**: PAC 89 / SHO 88 / PAS 86 / DRI 90 / DEF 45 / PHY 76

### Omar Marmoush (OVR 84)
- **ポジション**: FW  |  **クラブ**: Manchester City
- **年齢**: 27  |  **代表キャップ**: 48  |  **代表得点**: 11
- **能力値**: PAC 89 / SHO 85 / PAS 76 / DRI 86 / DEF 34 / PHY 71

### Mohamed Abdelmonem (OVR 74)
- **ポジション**: DF  |  **クラブ**: Nice
- **年齢**: 27  |  **代表キャップ**: 35  |  **代表得点**: 3
- **能力値**: PAC 62 / SHO 40 / PAS 63 / DRI 61 / DEF 74 / PHY 74

### Ramy Rabia (OVR 67)
- **ポジション**: DF  |  **クラブ**: Al Ain
- **年齢**: 33  |  **代表キャップ**: 44  |  **代表得点**: 5
- **能力値**: PAC 60 / SHO 49 / PAS 55 / DRI 55 / DEF 67 / PHY 72

---
## 3. 戦術的特徴

### 主フォーメーション: 4-3-3
> 3トップで幅を使う攻撃的布陣。ウイングの突破力とボール保持率が鍵。

**使用フォーメーション分布:**
- `4-3-3` : 7回
- `4-4-2` : 5回
- `4-4-1-1` : 1回
- `4-2-3-1` : 1回

### 主な先発メンバー（StatsBomb実績）

- Ahmed Elsayed Ali Elsayed Hegazy
- Mohamed Abdelmonem
- Hamdi Fathi
- Mostafa Mohamed Ahmed Abdallah
- Mohamed Hany Gamal Eldemerdash
- Mohamed Naser Elsayed Elneny
- Ahmed Sayed
- Mohamed El Sayed Mohamed El Shenawy Gomaa
- Ahmed Mohamed Abo El Fotouh Mohamed
- Marwan Attia Fahim Ghallab
- Mahmoud Ibrahim Hassan

### よく途中出場する選手

- Mostafa Fathi
- Mohamed Hamdy Sharaf
- Omar Marmoush
- Omar Kamal Abdel Wahed
- Mahmoud Ibrahim Hassan
- Mahmoud Hamada

### 近年の主な試合結果

| 大会 | 対戦相手 | スコア | フォーメーション |
|------|----------|:------:|:---------------:|
| AFCON 2023 | Congo DR | 1-1 | 4-3-3 |
| AFCON 2023 | Ghana | 2-2 | 4-3-3 |
| AFCON 2023 | Mozambique | 2-2 | 4-2-3-1 |
| AFCON 2023 | Cape Verde Islands | 2-2 | 4-3-3 |

---
## 4. 試合展開の有利・不利

### ✅ 有利になりやすい状況

- 3トップのプレス強度が高く、相手のビルドアップを高い位置で潰せる
- 両ウイングの突破力があれば、サイドを起点に連続してゴールチャンスを作れる
- FW陣に個人打開できる選手（最高OVR 91）がおり、1対1でも差を作れる

### ⚠️ 不利になりやすい状況

- ウイングが守備に戻らないと、サイドの背後に大きなスペースができ相手SBに使われる
- 中盤が1枚少ない構造のため、相手が中盤を厚くしてくると数的不利になりやすい
- DF陣の能力値が低めで、スピードのある相手FWに裏を取られる危険性がある

---
## 5. グループステージ展望

グループ **G** の対戦相手: Belgium, IR Iran, New Zealand

_（予測スコアは schedule.json 入力後に更新予定）_
