# Ecuador (ECU) — チーム分析レポート

> **グループ E** ｜ 主戦フォーメーション: `4-2-3-1` ｜ 平均OVR: **72.2** ｜ 分析試合数: 7試合 (StatsBomb)

---
## 1. 選手層

### ゴールキーパー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Hernán Galíndez | Aucas | **74** | None | None | None | None | None | None |
| Alexander Domínguez | Liga de Quito | **73** | None | None | None | None | None | None |
| Moisés Ramírez | Independiente del Valle | **—** | — | — | — | — | — | — |

### ディフェンダー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Piero Hincapié | Bayer Leverkusen | **83** | 84 | 41 | 65 | 72 | 84 | 82 |
| Angelo Preciado | Genk | **75** | 87 | 62 | 70 | 76 | 66 | 71 |
| Diego Palacios | Portland Timbers | **71** | 75 | 73 | 57 | 66 | 33 | 66 |
| Robert Arboleda | São Paulo | **—** | — | — | — | — | — | — |
| Félix Torres | Santos Laguna | **—** | — | — | — | — | — | — |
| Jackson Porozo | Espanyol | **—** | — | — | — | — | — | — |

### ミッドフィールダー

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Moisés Caicedo 🅲 | Chelsea | **87** | 71 | 64 | 78 | 81 | 84 | 82 |
| Carlos Gruezo | Augsburg | **72** | 58 | 46 | 62 | 70 | 67 | 75 |
| Jhegson Méndez | Los Angeles FC | **70** | 68 | 57 | 67 | 70 | 65 | 67 |
| Jeremy Sarmiento | Brighton | **70** | 78 | 63 | 65 | 74 | 37 | 59 |
| Ángel Mena | León | **—** | — | — | — | — | — | — |

### フォワード

| 選手名 | クラブ | OVR | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Kendry Páez | Independiente del Valle | **73** | 79 | 68 | 69 | 75 | 43 | 54 |
| Michael Estrada | Cruz Azul | **69** | 76 | 62 | 51 | 66 | 24 | 77 |
| Kevin Rodríguez | Ipswich Town | **67** | 79 | 65 | 54 | 68 | 25 | 67 |
| Enner Valencia | Internacional | **55** | 78 | 36 | 43 | 52 | 47 | 48 |
| Djorkaeff Reasco | Nantes | **—** | — | — | — | — | — | — |
| Gonzalo Plata | Al-Qadsiah | **—** | — | — | — | — | — | — |

---
## 2. キープレーヤー

### Moisés Caicedo (OVR 87)
- **ポジション**: MF  |  **クラブ**: Chelsea
- **年齢**: 24  |  **代表キャップ**: 43  |  **代表得点**: 4
- **能力値**: PAC 71 / SHO 64 / PAS 78 / DRI 81 / DEF 84 / PHY 82

### Piero Hincapié (OVR 83)
- **ポジション**: DF  |  **クラブ**: Bayer Leverkusen
- **年齢**: 24  |  **代表キャップ**: 38  |  **代表得点**: 2
- **能力値**: PAC 84 / SHO 41 / PAS 65 / DRI 72 / DEF 84 / PHY 82

### Angelo Preciado (OVR 75)
- **ポジション**: DF  |  **クラブ**: Genk
- **年齢**: 28  |  **代表キャップ**: 40  |  **代表得点**: 1
- **能力値**: PAC 87 / SHO 62 / PAS 70 / DRI 76 / DEF 66 / PHY 71

### Hernán Galíndez (OVR 74)
- **ポジション**: GK  |  **クラブ**: Aucas
- **年齢**: 39  |  **代表キャップ**: 45  |  **代表得点**: 0
- **能力値**: PAC None / SHO None / PAS None / DRI None / DEF None / PHY None

### Alexander Domínguez (OVR 73)
- **ポジション**: GK  |  **クラブ**: Liga de Quito
- **年齢**: 38  |  **代表キャップ**: 77  |  **代表得点**: 0
- **能力値**: PAC None / SHO None / PAS None / DRI None / DEF None / PHY None

---
## 3. 戦術的特徴

### 主フォーメーション: 4-2-3-1
> 守備的MF2枚を軸に、3列目に広さを持つバランス型。サイドアタックと中盤のプレス強度が高い。

**使用フォーメーション分布:**
- `4-2-3-1` : 11回
- `4-4-2` : 4回
- `3-4-2-1` : 2回
- `4-3-3` : 1回

### 主な先発メンバー（StatsBomb実績）

- Felix Eduardo Torres Caicedo
- Piero Martín Hincapié Reyna
- Angelo Smit Preciado Quiñónez
- Moisés Isaac Caicedo Corozo
- Enner Remberto Valencia Lastra
- Alan Steven Franco Palma
- Alexander Domínguez Carabalí
- Willian Joel Pacho Tenorio
- Ray Kendry Páez Andrade
- Jeremy Leonel Sarmiento Morante
- Hernán Ismael Galíndez

### よく途中出場する選手

- Kevin José Rodríguez Cortez
- Alan Steve Minda García
- Jeremy Leonel Sarmiento Morante
- Jordy Josué Caicedo Medina
- Carlos Armando Gruezo Arboleda
- José Adoni Cifuentes Charcopa

### 近年の主な試合結果

| 大会 | 対戦相手 | スコア | フォーメーション |
|------|----------|:------:|:---------------:|
| FIFA World Cup 2022 | Netherlands | 1-1 | 3-4-2-1 |
| FIFA World Cup 2022 | Senegal | 1-2 | 4-3-3 |
| FIFA World Cup 2022 | Qatar | 2-0 | 4-4-2 |
| Copa America 2024 | Jamaica | 3-1 | 4-2-3-1 |
| Copa America 2024 | Mexico | 0-0 | 4-4-2 |
| Copa America 2024 | Venezuela | 1-2 | 4-2-3-1 |
| Copa America 2024 | Argentina | 1-1 | 4-2-3-1 |

---
## 4. 試合展開の有利・不利

### ✅ 有利になりやすい状況

- 2ボランチによる守備的な安定感があり、ボール奪取後の素早い縦パスでカウンターを発動しやすい
- 3列目の横幅を使い、サイドチェンジで相手ブロックをこじ開けることができる

### ⚠️ 不利になりやすい状況

- 相手が5バックを敷くと中盤が窮屈になり、ビルドアップが詰まりやすい
- 1トップの孤立が起きると前線の起点が失われ、攻撃が停滞する
- GKのクオリティに不安があり、ミドルシュートやセットプレーで失点しやすい

---
## 5. グループステージ展望

グループ **E** の対戦相手: Germany, Curaçao, Côte d'Ivoire

_（予測スコアは schedule.json 入力後に更新予定）_
