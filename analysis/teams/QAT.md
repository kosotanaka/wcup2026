# Qatar (QAT) — チーム分析レポート

> **グループ B** ｜ 主戦フォーメーション: `3-5-2` ｜ 平均OVR: **69.2** ｜ 分析試合数: 3試合 (StatsBomb)

---
## 1. 選手層

### ゴールキーパー

| 選手名 | クラブ | OVR | POT | ↑ | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Meshaal Barsham | Al-Sadd | **70** | 74 | +4 | — | — | — | — | — | — |
| Salah Zakaria | Al-Duhail | **64** | 69 | +5 | — | — | — | — | — | — |
| Mahmud Abunada | Al-Rayyan | — | — | — | — | — | — | — | — | — |
| Shehab Ellethy | Al-Shahaniya | — | — | — | — | — | — | — | — | — |

### ディフェンダー

| 選手名 | クラブ | OVR | POT | ↑ | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Boualem Khoukhi | Al-Sadd | **71** | 71 | — | 60 | 60 | 57 | 54 | 71 | 76 |
| Lucas Mendes | Al-Wakrah | **70** | 70 | — | 53 | 57 | 59 | 59 | 70 | 71 |
| Pedro Miguel | Al-Sadd | **68** | 68 | — | 73 | 38 | 60 | 61 | 64 | 78 |
| Homam Ahmed | Cultural Leonesa | **68** | 71 | +3 | 76 | 48 | 60 | 61 | 62 | 70 |
| Sultan Al-Brake | Al-Duhail | — | — | — | — | — | — | — | — | — |
| Al-Hashmi Al-Hussain | Al-Arabi | — | — | — | — | — | — | — | — | — |
| Ayoub Al-Oui | Al-Gharafa | — | — | — | — | — | — | — | — | — |
| Issa Laye | Al-Arabi | — | — | — | — | — | — | — | — | — |
| Rayyan Al-Ali | Al-Gharafa | — | — | — | — | — | — | — | — | — |

### ミッドフィールダー

| 選手名 | クラブ | OVR | POT | ↑ | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Karim Boudiaf | Al-Duhail | **71** | 71 | — | 50 | 62 | 71 | 69 | 67 | 74 |
| Abdulaziz Hatem | Al-Rayyan | **70** | 70 | — | 59 | 67 | 70 | 68 | 61 | 70 |
| Assim Madibo | Al-Wakrah | **65** | 66 | +1 | 82 | 38 | 53 | 61 | 65 | 75 |
| Jassem Gaber | Al-Rayyan | **63** | 74 | +11 | 64 | 44 | 53 | 54 | 62 | 72 |
| Ahmed Fathy | Al-Arabi | — | — | — | — | — | — | — | — | — |
| Mohamed Al-Mannai | Al-Shamal | — | — | — | — | — | — | — | — | — |
| Tahsin Jamshid | Al-Duhail | — | — | — | — | — | — | — | — | — |

### フォワード

| 選手名 | クラブ | OVR | POT | ↑ | PAC | SHO | PAS | DRI | DEF | PHY |
|--------|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Akram Afif | Al-Sadd | **78** | 78 | — | 87 | 75 | 72 | 79 | 32 | 55 |
| Edmilson Junior | Al-Duhail | **76** | 76 | — | 88 | 75 | 70 | 79 | 44 | 65 |
| Almoez Ali | Al-Duhail | **74** | 74 | — | 75 | 74 | 58 | 68 | 25 | 72 |
| Hassan Al-Haydos 🅲 | Al-Sadd | **71** | 71 | — | 76 | 66 | 69 | 75 | 34 | 43 |
| Ahmed Al-Ganehi | Al-Gharafa | **67** | 71 | +4 | 72 | 62 | 60 | 68 | 25 | 51 |
| Mohammed Muntari | Al-Gharafa | **66** | 66 | — | 63 | 66 | 48 | 60 | 27 | 69 |
| Yusuf Abdurisag | Al-Wakrah | **65** | 66 | +1 | 82 | 56 | 57 | 68 | 30 | 51 |
| Ahmed Alaaeldin | Al-Rayyan | — | — | — | — | — | — | — | — | — |

---
## 2. キープレーヤー

### Akram Afif (OVR 78)
- **ポジション**: FW  |  **クラブ**: Al-Sadd
- **年齢**: 29  |  **代表キャップ**: 124  |  **代表得点**: 39
- **能力値**: PAC 87 / SHO 75 / PAS 72 / DRI 79 / DEF 32 / PHY 55

### Edmilson Junior (OVR 76)
- **ポジション**: FW  |  **クラブ**: Al-Duhail
- **年齢**: 31  |  **代表キャップ**: 15  |  **代表得点**: 0
- **能力値**: PAC 88 / SHO 75 / PAS 70 / DRI 79 / DEF 44 / PHY 65

### Almoez Ali (OVR 74)
- **ポジション**: FW  |  **クラブ**: Al-Duhail
- **年齢**: 29  |  **代表キャップ**: 115  |  **代表得点**: 55
- **能力値**: PAC 75 / SHO 74 / PAS 58 / DRI 68 / DEF 25 / PHY 72

### Boualem Khoukhi (OVR 71)
- **ポジション**: DF  |  **クラブ**: Al-Sadd
- **年齢**: 35  |  **代表キャップ**: 115  |  **代表得点**: 20
- **能力値**: PAC 60 / SHO 60 / PAS 57 / DRI 54 / DEF 71 / PHY 76

### Karim Boudiaf (OVR 71)
- **ポジション**: MF  |  **クラブ**: Al-Duhail
- **年齢**: 35  |  **代表キャップ**: 117  |  **代表得点**: 5
- **能力値**: PAC 50 / SHO 62 / PAS 71 / DRI 69 / DEF 67 / PHY 74

---
## 3. 戦術的特徴

### 主フォーメーション: 3-5-2
> 5人の中盤でピッチを制圧。5レーンすべてを使う幅広い攻撃とプレッシング強度が高い。

**使用フォーメーション分布:**
- `3-5-2` : 10回

### 主な先発メンバー（StatsBomb実績）

- Pedro Miguel Correia
- Boualem Khoukhi
- Abdelkarim Hassan Al Haj Fadlalla
- Homam Alamin Ahmed
- Hassan Khalid Al Heidos
- Almoez Ali Zainalabiddin Abdulla
- Akram Hassan Afif
- Meshaal Aissa Barsham
- Ismaeel Mohammad Mohammad
- Assim Omer Al Haj Madibo
- Abdulaziz Hatem Mohammed Abdullah

### よく途中出場する選手

- Mohammed Muntari
- Mohammed Waed Abdulwahhab Al Bayati
- Ali Assadalla Thaimn Qambar
- Karim Boudiaf
- Ahmed Alaa Eldin Abdelmotaal
- Mosaab Khoder Jibril

### 近年の主な試合結果

| 大会 | 対戦相手 | スコア | フォーメーション |
|------|----------|:------:|:---------------:|
| FIFA World Cup 2022 | Netherlands | 0-2 | 3-5-2 |
| FIFA World Cup 2022 | Senegal | 1-3 | 3-5-2 |
| FIFA World Cup 2022 | Ecuador | 0-2 | 3-5-2 |

---
## 4. 試合展開の有利・不利

### ✅ 有利になりやすい状況

- 5中盤でピッチ全幅をカバーし、どの位置でもボールを奪いやすい
- 2CFが競り合いに強ければ、前線でタメを作りつつ中盤の飛び出しを引き出せる

### ⚠️ 不利になりやすい状況

- 格上相手との個人能力差は大きく、ブロック守備と組織力で補う必要がある
- 相手の3バックに対し、2トップの枚数が足りず前線プレスが機能しない場面がある
- サイドに引っ張り出されると、DF3枚の横スライドに限界が生じる
- DF陣の能力値が低めで、スピードのある相手FWに裏を取られる危険性がある
- GKのクオリティに不安があり、ミドルシュートやセットプレーで失点しやすい

---
## 5. グループステージ展望

グループ **B** の対戦相手: Canada, Bosnia & Herzegovina, Switzerland

_（予測スコアは schedule.json 入力後に更新予定）_
